...
\end{enumerate}
donde $S_c = S3;S4$. Para demostar el punto 2, debemos usar el ``Teorema del Invariante'' y el ``Teorema de terminación de un ciclo". Ambos 
se cumplen bajo estas condiciones:
\begin{enumerate}
...
Sea $ciudades = \TLista{Ciudad}$ tal que cumple el requiere de nuestra especificación. Como cumple con esto último, se
tiene que existe un $j$, donde $0 \leq j < \abs{ciudades}$, tal que $ciudades[j]_1 > 50000$. Al pasar por el programa
$S$ (que es valido y termina), se tiene que
\begin{align*}
    ...
    res &> 50000
\end{align*}
con lo cual, el valor devuelto ($res$) es mayor a 50000.
\end{document}